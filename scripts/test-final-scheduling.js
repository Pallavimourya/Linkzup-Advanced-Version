...
/**
 * Final test for scheduling flow with timezone fixes
 */

const { MongoClient, ObjectId } = require('mongodb');

async function testFinalScheduling() {
  ...
  try {
    ...
    console.log(`📅 Scheduled time (IST): ${scheduledIST.toLocaleString('en-IN', {timeZone: 'Asia/Kolkata'})}`);
    
    // Convert IST to UTC for storage (this is what the modal does)
    const scheduledUTC = new Date(scheduledIST.getTime() - (5.5 * 60 * 60 * 1000));
    console.log(`🌍 UTC time (stored): ${scheduledUTC.toISOString()}`);
    
    // Verify the conversion
    const backToIST = new Date(scheduledUTC.getTime() + (5.5 * 60 * 60 * 1000));
    console.log(`🔄 Back to IST: ${backToIST.toLocaleString('en-IN', {timeZone: 'Asia/Kolkata'})}`);

    const testPost = {
      userId: testUser._id,
      userEmail: testUser.email,
      content: `🧪 Final timezone test - Scheduled for 12:54 PM IST - ${new Date().toISOString()}`,
      images: [],
    ...
    console.log(`   Scheduled For (IST): ${createdPost.scheduledFor.toLocaleString('en-IN', {timeZone: 'Asia/Kolkata'})}`);
...
// Run the test
testFinalScheduling().catch(console.error);
