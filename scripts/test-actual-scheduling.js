...
/**
 * Test script to verify actual scheduling flow with timezone fixes
 */
...
async function testActualScheduling() {
  ...
  try {
    ...
    // Test: Schedule a post for 12:54 PM IST (the time user mentioned)
    ...
    console.log(`📅 Scheduled time (IST): ${formatIstDateShort(scheduledIST)} at ${formatIstTime(scheduledIST)}`);
    ...
    const backToIST = utcToIst(scheduledUTC);
    console.log(`🔄 Back to IST: ${formatIstDateShort(scheduledUTC)} at ${formatIstTime(scheduledUTC)}`);

    const testPost = {
      userId: testUser._id,
      userEmail: testUser.email,
      content: `🧪 Timezone test - Scheduled for 12:54 PM IST - ${new Date().toISOString()}`,
      images: [],
    ...
    const actualTime = formatIstTime(createdPost.scheduledFor);
...
// Run the test
testActualScheduling().catch(console.error);
