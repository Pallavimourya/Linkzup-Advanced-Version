  ...
  try {
    ...
    if (!type || !topicTitle) {
      return NextResponse.json({ 
        error: "Type and topicTitle are required",
    ...
    // Generate topic-based content using the topic generator method (WITH personal story)
    const response = await aiService.generateTopicContent(
      type,
      topicTitle,
      provider,
      customization,
      session.user.id,
      session.user.email // Include userEmail for personal story integration
    )
    ...
    return NextResponse.json({
      ...
      data: response,
      queue: queueStatus,
      message: `Successfully generated topic-based ${type} content using ${provider}`,
      approach: "topic_personal_story", // Indicate this is topic + personal story approach
      topicTitle: topicTitle
    })

  } catch (error) {
    console.error("Error in topic-based content generation API:", error)
    
    return NextResponse.json({ 
      error: "Failed to generate topic-based content",
