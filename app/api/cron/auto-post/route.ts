...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if this is a specific post request
    ...
    if (postId && userId) {
      ...
      const post = await db.collection("scheduled_posts").findOne({
      ...
      // Process all due posts (fallback for existing cron jobs)
      const now = new Date()
      const fiveMinutesAgo = new Date(now.getTime() - 5 * 60 * 1000)

      postsToProcess = await db
    ...
    for (const post of postsToProcess) {
      try {
        ...
        try {
          // Try to convert to ObjectId first
          userIdQuery = new ObjectId(post.userId)
        } catch (error) {
          // If conversion fails, use string as fallback
          userIdQuery = post.userId
        }
        ...
        // For cron jobs, we need to post directly without session
