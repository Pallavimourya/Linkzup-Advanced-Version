...
Now, generate the most natural, story-based, and specific blog topics for the story below:`
    ...
    if (Array.isArray(response.content)) {
      ...
      topics = response.content
        ...
        .slice(0, 3) // Changed from 5 to 3 topics
    }
...
Now, generate 3 NEW, natural, story-based, and specific blog topics that are different from the existing ones above:
...
Generation timestamp: ${new Date().toISOString()}`
    ...
    if (topics.length < 3) {
      ...
      topics = [...topics, ...newFallbackTopics].slice(0, 3)
    }
