...
export async function POST(request: NextRequest) {
  try {
    console.log("=== REGENERATE PERSONALIZED TOPICS API CALLED ===")
    ...
    if (!isStoryComplete) {
      console.log("Story not complete, returning false")
      return NextResponse.json({ 
        hasPersonalStory: false,
    ...
    try {
      ...
      console.log("Story data retrieved:", storyData ? "Yes" : "No")
    } catch (error) {
    ...
    // Force delete existing topics to ensure regeneration
    try {
      const db = await connectDB()
      await db.collection("personalizedTopics").deleteOne({ userEmail })
      console.log("Deleted existing topics to force regeneration")
    } catch (error) {
      console.error("Error deleting existing topics:", error)
      // Continue with generation even if deletion fails
    }

    // Generate detailed topics with keywords from answers
    console.log("Generating detailed personalized topics with keywords...")
    const topics = generateDetailedTopics(storyData)
    
    console.log("Generated topics count:", topics.length)
    console.log("First few topics:", topics.slice(0, 3))
    ...
    return NextResponse.json({
      ...
      topics: topics,
      message: "Personalized topics regenerated and stored successfully",
      isStored: false
    })

  } catch (error) {
    console.error("Error regenerating personalized topics:", error)
    return NextResponse.json(
      { error: "Failed to regenerate personalized topics" },
      { status: 500 }
    )
  }
}
